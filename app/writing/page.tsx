...
export default function WritingPage() {
  ...
  const handleArticleSelect = async (article: NewsArticle) => {
    ...
    try {
      // 기사 전체 내용 가져오기 (최대 8000자)
      let fullContent = article.content
      
      // content가 짧거나 불완전해 보이면 fetch-article API로 전체 내용 가져오기 시도
      // 더 적극적으로 전체 내용 가져오기 (2000자 미만이거나 "+chars" 같은 패턴이 있으면)
      if ((article.content.length < 2000 || article.content.includes('+chars') || article.content.includes('[+')) && article.id) {
        try {
